...
var карта: = preload("res://Карта3D.tscn")  
var preview = true
...
var КриваяВРуке_X: Curve = load("res://Основа/позиция_карт_в_руке_X.tres")
var КриваяВРуке_Z: Curve = load("res://Основа/позиция_карт_в_руке_Z.tres")
var КриваяВРуке_поворот: Curve = load("res://Основа/позиция_карт_в_руке_поворот.tres")
...
func взять_карту(сколько_взять: int):
	...
	for взятие_карты in сколько_взять:
		if not ИгроваяКолода.is_empty():
			...
			var айди
			айди = ИгроваяКолода[0]
			var ID = айди.get("id")
			новая_карта.создание_карты(ID, Data.дата_карт[ID]["name"], Data.дата_карт[ID]["описание"], load(Data.дата_карт[ID]["тип"]),
			Data.дата_карт[ID]["редкость"], Data.дата_карт[ID]["эффекты"], Data.дата_карт[ID]["стоимость"], Data.дата_карт[ID]["прочность"],
			Data.дата_карт[ID]["атака"], Data.дата_карт[ID]["здоровье"], Data.дата_карт[ID]["время жизни"])
